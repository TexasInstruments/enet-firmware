...
ifneq ($(filter $($(1)_CPU),x86 X86 i386 i486 i586 i686),)
    ...
    $(1)_ARCH=64
    $(1)_ENDIAN=LITTLE
else ifneq ($(filter $($(1)_CPU),ARM M3 M4 A8 A8F A9 A9F A15 A15F armv7l R5F R5Ft),)
    ...
    $(1)_ARCH=32
    $(1)_ENDIAN=LITTLE
else ifneq ($(filter $($(1)_CPU),ARM64 aarch64 A53 A54 A57 A72),)
    HOST_PLATFORM=PC
    $(1)_FAMILY=ARM
    $(1)_ARCH=64
    $(1)_ENDIAN=LITTLE
else ifneq ($(filter $($(1)_CPU),C6XSIM C64T C64P C64 C66 C674 C67 C67P C71),)
    HOST_PLATFORM=PC
    $(1)_FAMILY=DSP
    $(1)_ARCH=32
    ifeq ($(TARGET_CPU),C71)
