...
$(_MODULE)_STRIP  := $(TDIR)/$(BIN_PRE)$($(_MODULE)_TARGET)$(STRIP_EXT)$(BIN_EXT)
...
$(_MODULE)_OBJS := $(ASSEMBLY:%.asm=$(ODIR)/%.$(OBJ_EXT)) $(CPPSOURCES:%.cpp=$(ODIR)/%.$(OBJ_EXT)) $(CSOURCES:%.c=$(ODIR)/%.$(OBJ_EXT))
...
ifeq ($(TARGET_OS),SYSBIOS)
	ifeq ($(TARGET_CPU),R5F)
	$(_MODULE)_XDC_TARGET := ti.targets.arm.elf.R5F
	else ifeq ($(TARGET_CPU),R5Ft)
	$(_MODULE)_XDC_TARGET := ti.targets.arm.elf.R5Ft
	endif
endif
...
ifneq ($(filter $(TARGET_CPU),A8 A8F),)
	...
	$(_MODULE)_COPT +=-mv=7R5 --float_support=vfpv3d16 --code_state=32
else ifeq ($(TARGET_CPU),R5Ft)
	$(_MODULE)_COPT +=-mv=7R5 --float_support=vfpv3d16 --code_state=16
endif
...
ifeq ($(TARGET_OS),SYSBIOS)
	ifeq ($(TARGET_CPU),R5F)
	$(_MODULE)_DEFS += xdc_target_name__=ti/targets/arm/elf/R5F
	else ifeq ($(TARGET_CPU),R5Ft)
	$(_MODULE)_DEFS += xdc_target_name__=ti/targets/arm/elf/R5Ft
	endif
